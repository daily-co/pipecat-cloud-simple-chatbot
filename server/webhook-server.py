...
class RoomRequest(BaseModel):
    ...
    callId: Optional[str] = Field(
        None, alias="call_id", description="Unique call identifier"
    )
    ...
    call_transfer: Optional[Dict[str, Any]] = Field(
        None, description="to initiate a call transfer"
    )
...
async def dial(request: RoomRequest, raw_request: Request):
    ...
    if all(
        field in request.model_dump() and request.model_dump()[field] is not None
